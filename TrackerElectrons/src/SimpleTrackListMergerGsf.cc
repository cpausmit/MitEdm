...
//    using namespace reco;
    ...
    if (trackCollection1.isValid()) {
      ...
      TC1 = &s_empty1;
      edm::LogWarning("SimpleTrackListMergerGsf") << "1st GsfTrackCollection " << trackProducer1 << " not found; will only clean 2nd GsfTrackCollection " << trackProducer2 ;
    }
    ...
    if (trackCollection2.isValid()) {
      ...
      //std::cout << "2nd collection " << trackProducer2 << " has "<< TC2->size() << " tracks" << std::endl ;
        ...
        TC2 = &s_empty2;
        edm::LogWarning("SimpleTrackListMergerGsf") << "2nd GsfTrackCollection " << trackProducer2 << " not found; will only clean 1st GsfTrackCollection " << trackProducer1 ;
    }
    ...
    outputGsfTrkExtras = std::auto_ptr<reco::GsfTrackExtraCollection>(new reco::GsfTrackExtraCollection);
  ...
  // 
   ...
   if ( 0<tC1.size() ){
      ...
      for (reco::GsfTrackCollection::const_iterator track=tC1.begin(); track!=tC1.end(); track++){
        ...
        if ((short unsigned)track->ndof() < 1){
          selected1[i]=0; 
          //std::cout << "L1Track "<< i << " rejected in SimpleTrackListMergerGsf; ndof() < 1" << std::endl ;
          continue;
        }
        if (track->normalizedChi2() > maxNormalizedChisq){
          selected1[i]=0; 
          //std::cout << "L1Track "<< i << " rejected in SimpleTrackListMergerGsf; normalizedChi2() > maxNormalizedChisq " << track->normalizedChi2() << " " << maxNormalizedChisq << std::endl ;
          continue;
        }
        if (track->found() < minFound){
          selected1[i]=0; 
          //std::cout << "L1Track "<< i << " rejected in SimpleTrackListMergerGsf; found() < minFound " << track->found() << " " << minFound << std::endl ;
          continue;
        }
        if (track->pt() < minPT){
          selected1[i]=0; 
          //std::cout << "L1Track "<< i << " rejected in SimpleTrackListMergerGsf; pt() < minPT " << track->pt() << " " << minPT << std::endl ;
          continue;
        }
   ...
   if ( 0<tC2.size() ){
      ...
      for (reco::GsfTrackCollection::const_iterator track=tC2.begin(); track!=tC2.end(); track++){
        ...
        if ((short unsigned)track->ndof() < 1){
          selected2[i]=0; 
          //std::cout << "L2Track "<< i << " rejected in SimpleTrackListMergerGsf; ndof() < 1" << std::endl ;
          continue;
        }
        if (track->normalizedChi2() > maxNormalizedChisq){
          selected2[i]=0; 
          //std::cout << "L2Track "<< i << " rejected in SimpleTrackListMergerGsf; normalizedChi2() > maxNormalizedChisq " << track->normalizedChi2() << " " << maxNormalizedChisq << std::endl ;
          continue;
        }
        if (track->found() < minFound){
          selected2[i]=0; 
          //std::cout << "L2Track "<< i << " rejected in SimpleTrackListMergerGsf; found() < minFound " << track->found() << " " << minFound << std::endl ;
          continue;
        }
        if (track->pt() < minPT){
          selected2[i]=0; 
          //std::cout << "L2Track "<< i << " rejected in SimpleTrackListMergerGsf; pt() < minPT " << track->pt() << " " << minPT << std::endl ;
          continue;
        }
      }//end loop over tracks
   }//end more than 0 track
  ...
  //
   if ( 1<tC1.size() ){
    ...
    for (reco::GsfTrackCollection::const_iterator track=tC1.begin(); track!=tC1.end(); track++){
      ...
      int j=-1;
      for (reco::GsfTrackCollection::const_iterator track2=tC1.begin(); track2!=tC1.end(); track2++){
        j++;
  ...
  //
   if ( 1<tC2.size() ){
    ...
    for (reco::GsfTrackCollection::const_iterator track=tC2.begin(); track!=tC2.end(); track++){
      ...
      int j=-1;
      for (reco::GsfTrackCollection::const_iterator track2=tC2.begin(); track2!=tC2.end(); track2++){
        j++;
  ...
  //
   ...
   if ( 0<tC1.size() ){
	...
	outputTrks->back().setQuality(qualityToSet);
      ...
      outputTrks->back().setGsfExtra( reco::GsfTrackExtraRef( refGsfTrkExtras, outputGsfTrkExtras->size() - 1) );
