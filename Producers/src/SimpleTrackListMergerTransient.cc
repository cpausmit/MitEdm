...
//    using namespace reco;
    ...
    // this allows SimpleTrackListMergerTransient to be used as a cleaner only if handed just one list
    ...
    if (trackCollection1.isValid()) {
      ...
      TC1 = &s_empty1;
      edm::LogWarning("SimpleTrackListMergerTransient") << "1st TrackCollection " << trackProducer1 << " not found; will only clean 2nd TrackCollection " << trackProducer2 ;
    }
    ...
    if (trackCollection2.isValid()) {
      ...
      //std::cout << "2nd collection " << trackProducer2 << " has "<< TC2->size() << " tracks" << std::endl ;
        ...
        TC2 = &s_empty2;
        edm::LogWarning("SimpleTrackListMergerTransient") << "2nd TrackCollection " << trackProducer2 << " not found; will only clean 1st TrackCollection " << trackProducer1 ;
    }
   ...
   if ( 0<tC1.size() ){
	...
	const reco::Track *track = stable->track();
        ...
        if (track->normalizedChi2() > maxNormalizedChisq){
          selected1[i]=0; 
          //std::cout << "L1Track "<< i << " rejected in SimpleTrackListMergerTransient; normalizedChi2() > maxNormalizedChisq " << track->normalizedChi2() << " " << maxNormalizedChisq << std::endl ;
          continue;
        }
        if (track->found() < minFound){
          selected1[i]=0; 
          //std::cout << "L1Track "<< i << " rejected in SimpleTrackListMergerTransient; found() < minFound " << track->found() << " " << minFound << std::endl ;
          continue;
        }
        if (track->pt() < minPT){
          selected1[i]=0; 
          //std::cout << "L1Track "<< i << " rejected in SimpleTrackListMergerTransient; pt() < minPT " << track->pt() << " " << minPT << std::endl ;
          continue;
        }
   ...
   if ( 0<tC2.size() ){
	...
	const reco::Track *track = stable->track();
        ...
        if (track->normalizedChi2() > maxNormalizedChisq){
          selected2[i]=0; 
          //std::cout << "L2Track "<< i << " rejected in SimpleTrackListMergerTransient; normalizedChi2() > maxNormalizedChisq " << track->normalizedChi2() << " " << maxNormalizedChisq << std::endl ;
          continue;
        }
        if (track->found() < minFound){
          selected2[i]=0; 
          //std::cout << "L2Track "<< i << " rejected in SimpleTrackListMergerTransient; found() < minFound " << track->found() << " " << minFound << std::endl ;
          continue;
        }
        if (track->pt() < minPT){
          selected2[i]=0; 
          //std::cout << "L2Track "<< i << " rejected in SimpleTrackListMergerTransient; pt() < minPT " << track->pt() << " " << minPT << std::endl ;
          continue;
        }
      }//end loop over tracks
   }//end more than 0 track
  ...
  //
   ...
   if ( (0<tC1.size())&&(0<tC2.size()) ){
	...
	const reco::Track *track2 = stable2->track();
        if ((!selected2[j])||(!selected1[i])||(stable->pid()!=stable2->pid())||(!track2->extra().isAvailable()))continue;
